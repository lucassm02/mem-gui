...
const KeyList = () => {
  ...
  return (
    <div className={`w-full px-6 max-w-7xl mx-auto mt-10 transition-all ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      ...
      <div className={`p-3 rounded-lg mb-4 flex items-center justify-between ${darkMode ? 'bg-gray-800' : 'bg-gray-200'} shadow-md`}>
        <input
          type="text"
          ...
          <tbody>
            {filteredKeys.length > 0 ? (
              filteredKeys.slice(0, maxItems).map((item) => (
                <tr key={item.key} className={`border-b transition-all ${darkMode ? 'border-gray-700 hover:bg-gray-700' : 'border-gray-300 hover:bg-gray-100'}`}>
                  <td className={`px-6 py-4 truncate max-w-[300px]  ${darkMode ? 'text-gray-100' : 'text-gray-800'}`}>{item.key}</td>
                  <td className={`px-6 py-4 truncate max-w-[250px] ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{item.value}</td>
                  ...
                  <td className="px-6 py-4 truncate max-w-[300px] text-right flex justify-end gap-3">
                    <button onClick={() => openViewDataModal(item)} className="text-blue-400 hover:text-blue-300 cursor-pointer">
                      <DocumentMagnifyingGlassIcon className="w-5 h-5" />
                    </button>
                    <button onClick={() => openEditModal(item)} className="text-blue-400 hover:text-blue-300 cursor-pointer">
                      <PencilSquareIcon className="w-5 h-5" />
                    </button>
                    <button onClick={() => handleDeleteKey(item.key)} className="text-red-400 hover:text-red-300 cursor-pointer">
              ...
              <tr>
                <td colSpan="5" className="px-6 py-4 text-center text-gray-500">Nenhuma chave armazenada</td>
              </tr>
            )}
          </tbody>
        </table>
